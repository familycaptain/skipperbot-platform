"""Reminders — data layer (SQL CRUD).

Owns reads + writes for the ``app_reminders.reminders`` table.
Higher-level business logic (RRULE parsing, nag scheduling, snooze,
the scheduler loop) lives in ``apps/reminders/store.py`` and
``apps/reminders/scheduler.py``.

Ported from ``data_layer/reminders.py`` for sub-chunk 7c-part-1.
Functionally identical; only difference is routing all queries
through the ``*_in_schema`` helpers from ``app_platform.db`` so the
reminders app's table lands in (and reads from) the ``app_reminders``
schema.
"""
...
_REMINDER_HINT = (
    "Focus on: the user, the message, the next remind_at time, whether it "
    "recurs (recurrence RRULE), and whether it nags daily until acknowledged. "
    "Reminders are how chat answers 'what am I supposed to do today?'."
)
...
BACKFILL_ENTITIES = [
    {
        ...
        "list_fn": lambda: get_all_reminders(),
        "context_hint": _REMINDER_HINT,
    },
]
...
def save_reminder(r: dict):
    ...
    On conflict, every editable field is updated (message, remind_at,
    recurrence, active, nag, last_nagged, time_slot, sort_order,
    schedule_id) — matches source semantics.
    """
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                INSERT INTO reminders (id, user_id, message, remind_at, recurrence,
                                       active, nag, last_nagged, time_slot, created_at,
                                       sort_order, schedule_id)
...
def get_reminder(reminder_id: str) -> dict | None:
    row = fetch_one_in_schema(SCHEMA, "SELECT * FROM reminders WHERE id = %s", (reminder_id,))
...
def get_all_reminders() -> list[dict]:
    return [
        ...
        for r in fetch_all_in_schema(
            SCHEMA,
            "SELECT * FROM reminders ORDER BY sort_order, created_at",
        )
    ]
...
def get_user_reminders(user_id: str) -> list[dict]:
    return [
        ...
        for r in fetch_all_in_schema(
            SCHEMA,
            "SELECT * FROM reminders WHERE user_id = %s ORDER BY sort_order, created_at",
            (user_id,),
        )
    ]
...
def save_all_reminders(reminders: list[dict]):
    """Bulk save — used by the store's _save_reminders pattern."""
    with scoped_conn(SCHEMA) as conn:
        with conn.cursor() as cur:
            for r in reminders:
                cur.execute(
...
def next_sort_order() -> int:
    ...
    row = fetch_one_in_schema(
        SCHEMA,
        "SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_val FROM reminders",
    )
...
def reorder_reminder(
    ...
    Swaps sort_order with the adjacent *visible* reminder — honours
    the same user_id and active filters the frontend is currently
    showing so the swap always affects a neighbour the user can
    actually see.
    """
    current = fetch_one_in_schema(
        SCHEMA,
        "SELECT id, sort_order, nag FROM reminders WHERE id = %s",
        (reminder_id,),
    )
    if not current:
        return False
    ...
    with scoped_conn(SCHEMA) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE reminders SET sort_order = %s WHERE id = %s",
                (neighbor["sort_order"], reminder_id),
            )
            cur.execute(
                "UPDATE reminders SET sort_order = %s WHERE id = %s",
