    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
          className="bg-slate-800 text-xs text-slate-300 rounded px-2 py-1.5 border border-slate-700">
        ...
        {isParent && (
          <button onClick={() => setShowCreate(!showCreate)}
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-md hover:bg-indigo-500 shrink-0"
            title="New Bounty">
            <Plus size={13} /> <span className="hidden sm:inline">New Bounty</span>
          </button>
        )}
        <button onClick={load} className="p-1.5 text-slate-400 hover:text-white rounded-md hover:bg-slate-800 shrink-0"
          title="Refresh">
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-slate-500 text-sm">No bounties found.</div>
      ) : (
...
function CreateBountyForm({ categories, userId, onCreated, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-800/50 rounded-lg p-3 space-y-2 border border-slate-700">
      <div className="flex gap-2">
        <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
          placeholder="Bounty title" className="flex-1 min-w-0 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
        <input value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })}
          placeholder="$ value" type="number" step="0.01" min="0.01"
          className="w-20 sm:w-24 shrink-0 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
      </div>
      <div className="flex flex-wrap gap-2">
        <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
          className="bg-slate-900 text-sm text-slate-300 px-2 py-1.5 rounded border border-slate-700 max-w-full">
          <option value="">No category</option>
          {categories.map(c => <option key={c.id} value={c.name}>{c.icon} {c.name}</option>)}
        </select>
        <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
          placeholder="Description (optional)" className="flex-1 min-w-[10rem] bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
      </div>
  ...
  return (
    <div className="bg-slate-800/50 rounded-lg border border-slate-700 overflow-hidden">
      <div className="flex items-center gap-2 sm:gap-3 px-3 py-2.5 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="text-sm text-white font-medium flex-1 min-w-0 break-words">{b.title}</span>
        {b.category && <span className="hidden sm:inline text-[10px] text-slate-500 shrink-0">{b.category}</span>}
          ...
          {isParent && (
            <div className="flex justify-end gap-1">
              <button onClick={(e) => { e.stopPropagation(); setEditing(true); setConfirmDelete(false); }}
                className="flex items-center gap-1 px-2 py-1 text-[10px] text-slate-400 hover:text-white bg-slate-700/50 hover:bg-slate-700 rounded">
                <Pencil size={10} /> Edit
              </button>
              {b.status !== "approved" && (
                confirmDelete ? (
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); deleteBounty(); }} disabled={busy}
                      className="px-2 py-1 text-[10px] bg-red-600 text-white rounded hover:bg-red-500 disabled:opacity-50">Yes</button>
                    <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(false); }}
                      className="px-2 py-1 text-[10px] text-slate-400 hover:text-white bg-slate-700/50 hover:bg-slate-700 rounded">No</button>
                  </div>
                ) : (
                  <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(true); }}
                    className="flex items-center gap-1 px-2 py-1 text-[10px] text-slate-400 hover:text-red-400 bg-slate-700/50 hover:bg-slate-700 rounded">
          ...
          {(b.status === "open" || b.status === "submitted") && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 pt-1">
              <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)"
                className="w-full sm:flex-1 bg-slate-900 text-xs text-white px-2 py-1.5 rounded border border-slate-700" />
              <div className="flex items-center gap-2 flex-wrap">
                {b.status === "open" && (
                  <>
                    ...
                    {isParent && (
                      <button onClick={() => doAction("skip")} disabled={busy} title="Parent did this — skip and cool down"
                        className="flex items-center gap-1 px-3 py-1.5 text-xs bg-slate-600 text-slate-300 rounded hover:bg-slate-500 disabled:opacity-50">
                        <X size={11} /> Skip
                      </button>
                    )}
                  </>
                ...
                {b.status === "submitted" && isParent && (
                  <>
                    ...
                    <button onClick={() => doAction("reject")} disabled={busy}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs bg-red-600 text-white rounded hover:bg-red-500 disabled:opacity-50">
                      <X size={11} /> Reject
                    </button>
                  </>
                )}
...
function BalanceTab({ userId, userRole, refreshKey }) {
  ...
  return (
    <div className="p-4 space-y-4">
      {/* My balance */}
      {balance && (
        <div className="bg-gradient-to-br from-emerald-900/30 to-slate-800/50 rounded-lg p-4 border border-emerald-700/30">
      ...
      {isParent && allBalances.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-slate-400 uppercase mb-2">Family Balances</h3>
          <div className="space-y-1">
            {allBalances.map(b => (
              <div key={b.user_id} className="flex items-center gap-2 sm:gap-3 bg-slate-800/50 rounded px-3 py-2 border border-slate-700">
                <span className="text-sm text-white font-medium flex-1 min-w-0 truncate">{b.user_id}</span>
                <span className="text-sm font-bold text-emerald-400 shrink-0">{cents(b.balance_cents)}</span>
                <span className="hidden sm:inline text-[10px] text-slate-500 shrink-0">earned {cents(b.lifetime_earned_cents)}</span>
      ...
      {transactions.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-slate-400 uppercase mb-2">Recent Transactions</h3>
          <div className="space-y-1">
            {transactions.map(t => (
              <div key={t.id} className="flex items-center gap-2 text-xs px-3 py-1.5 bg-slate-800/30 rounded border border-slate-700/50">
                ...
                <span className="text-slate-400 flex-1 min-w-0 truncate">{t.note || t.type}</span>
                {t.payment_method && <span className="hidden sm:inline text-[10px] text-slate-500 shrink-0">{t.payment_method}</span>}
                <span className="text-[10px] text-slate-600 shrink-0">{new Date(t.created_at).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
...
function RecordPaymentForm({ userId: payeeId, recordedBy, onDone, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-800/50 rounded-lg p-3 space-y-2 border border-indigo-700/30">
      <div className="text-xs text-slate-300 font-medium">Record Payment to {payeeId}</div>
      {error && <div className="text-xs text-red-400">{error}</div>}
      <div className="flex flex-wrap gap-2">
        <input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="$ amount"
          type="number" step="0.01" min="0.01"
          className="w-24 sm:w-28 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
        <select value={method} onChange={(e) => setMethod(e.target.value)}
          className="bg-slate-900 text-sm text-slate-300 px-2 py-1.5 rounded border border-slate-700">
          <option value="cash">Cash</option>
          <option value="venmo">Venmo</option>
          <option value="zelle">Zelle</option>
        </select>
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note"
          className="flex-1 min-w-[8rem] bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
      </div>
      <div className="flex gap-2 justify-end">
        <button type="button" onClick={onCancel} className="px-3 py-1.5 text-xs text-slate-400">Cancel</button>
...
function LeaderboardTab({ refreshKey }) {
  ...
  const podiumStyles = [
    ...
    "bg-slate-700/40 border-slate-400/50 shadow-slate-800/20 shadow-sm",
    "bg-amber-900/20 border-amber-700/50",
  ];
  const valueStyles = ["text-yellow-300", "text-slate-200", "text-amber-400"];
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-slate-500 text-sm">No completions yet.</div>
      ) : (
        <div className="space-y-2">
          {leaders.map((entry, i) => (
            <div key={entry.user_id}
              ...
              <span className={`w-8 text-center ${i < 3 ? "text-2xl" : "text-sm text-slate-500"}`}>
                {medals[i] || `${i + 1}.`}
              </span>
              <span className={`text-sm font-medium flex-1 ${i < 3 ? "text-white" : "text-slate-300"}`}>
                {entry.user_id}
              </span>
              <div className="text-right">
                ...
                <div className="text-[10px] text-slate-500">{entry.bounties_completed} bounties</div>
              </div>
            </div>
          ))}
        </div>
...
function TemplatesTab({ userId, userRole, refreshKey }) {
  ...
  if (!isParent) {
    return <div className="p-4 text-slate-500 text-sm">Only parents can manage bounty templates.</div>;
  }

  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-2">
        <h3 className="text-xs font-semibold text-slate-400 uppercase flex-1">Bounty Templates (Recurring)</h3>
        <button onClick={() => setShowCreate(!showCreate)}
          className="flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-md hover:bg-indigo-500">
...
function CreateTemplateForm({ categories, userId, onCreated, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-800/50 rounded-lg p-3 space-y-2 border border-slate-700">
      <div className="flex gap-2">
        <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
          placeholder="Template title" className="flex-1 min-w-0 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
        <input value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })}
          placeholder="$ value" type="number" step="0.01" min="0.01"
          className="w-20 sm:w-24 shrink-0 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
      </div>
...
function TemplateCard({ template: t, categories = [], userId, onRefresh }) {
  ...
  if (editing) {
    return (
      <form onSubmit={handleSave} className="bg-slate-800/50 rounded-lg p-3 space-y-2 border border-indigo-700/30">
        <div className="flex gap-2">
          <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
            placeholder="Template title" className="flex-1 min-w-0 bg-slate-900 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
          <input value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })}
        ...
        <div className="flex flex-wrap gap-2 justify-end text-[10px]">
          <span className="text-slate-500 flex-1 self-center min-w-[8rem]">Changes propagate to open bounties</span>
          <button type="button" onClick={() => setEditing(false)} className="px-3 py-1.5 text-slate-400">Cancel</button>
          <button type="submit" disabled={saving || !form.title.trim() || !form.value}
            className="px-3 py-1.5 bg-indigo-600 text-white rounded-md hover:bg-indigo-500 disabled:opacity-50">
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
      </form>
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 sm:gap-3 px-3 py-2.5 rounded-lg border ${
      t.is_active ? "bg-slate-800/50 border-slate-700" : "bg-slate-900/50 border-slate-800 opacity-60"
    }`}>
      <div className="flex-1 min-w-[10rem] cursor-pointer" onClick={() => setEditing(true)} title="Click to edit">
        <div className="text-sm text-white font-medium break-words">{t.title}</div>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <button onClick={handleToggle} disabled={busy}
          className="px-2 py-1 text-[10px] bg-slate-700 text-slate-300 rounded hover:bg-slate-600">
...
function SettingsTab({ userId, userRole, refreshKey }) {
  ...
  if (!isParent) {
    return <div className="p-4 text-slate-500 text-sm">Only parents can change bounty settings.</div>;
  }
  ...
  return (
    <div className="p-4 space-y-6 max-w-lg">
      <div>
        <h3 className="text-xs font-semibold text-slate-400 uppercase mb-2">Minimum Payout Amount</h3>
        <div className="flex items-center gap-2">
          <span className="text-slate-400 text-sm">$</span>
          <input value={minPayout} onChange={(e) => setMinPayout(e.target.value)}
            type="number" step="0.01" min="0"
            className="w-28 bg-slate-800 text-sm text-white px-2 py-1.5 rounded border border-slate-700" />
        ...
        <p className="text-[10px] text-slate-500 mt-1">Balance must meet this threshold before a payment can be recorded.</p>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1 mb-2">
          {categories.map(c => (
            <div key={c.id} className="flex items-center gap-2 text-sm text-slate-300 bg-slate-800/50 rounded px-3 py-1.5 border border-slate-700">
              <span>{c.icon}</span>
        ...
        <div className="flex gap-2">
          <input value={newCat} onChange={(e) => setNewCat(e.target.value)} placeholder="New category name"
            className="flex-1 bg-slate-800 text-sm text-white px-2 py-1.5 rounded border border-slate-700"
