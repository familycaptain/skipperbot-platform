"""Schedules — notifier loop.

Checks for upcoming + overdue schedules and writes notification rows
through the ``app_platform.notifications`` shim. Called from the
reminders scheduler tick (~30s).

Two notification types:

- **UPCOMING** — ``next_due`` is within ``reminder_mins`` from now;
  fires once per occurrence (deduplicated by a 12-hour window).
- **OVERDUE** — ``next_due`` has passed; fires once per day at a
  deterministic random waking hour so the user isn't blasted at
  midnight.

Ported from ``schedule_notifier.py`` for sub-chunk 8e. Changes:

1. ``data_layer.schedules`` → ``apps.schedules.data``.
2. Raw-SQL dedup queries against ``public.notifications`` switched to
   ``app_platform.notifications.get_notifications(...)`` — the
   contract that survives schema moves. This also fixes a latent bug
   introduced in Chunk 6 (notifications packaging) where the
   ``public.notifications`` table moved to ``app_notifications.notifications``
   and the raw queries would have silently failed.
"""
...
def register_schedule_claim(linked_entity_type: str):
    ...
    The schedules notifier will skip schedules with this
    linked_entity_type — the claiming app is responsible for its own
    notifications.
    """
...
def _has_recent_notification(schedule_id: str, hours: int = 12) -> bool:
    ...
    rows = get_notifications(source_type="schedule", source_id=schedule_id, limit=1)
    if not rows:
        return False
    try:
...
def _has_overdue_notification_today(schedule_id: str) -> bool:
    ...
    rows = get_notifications(source_type="schedule_overdue", source_id=schedule_id, limit=1)
    if not rows:
        return False
    try:
        created = datetime.fromisoformat(rows[0]["created_at"])
...
async def check_schedule_notifications():
    ...
    try:
        ...
        schedules = await asyncio.to_thread(
            get_due_schedules, days_ahead=7, exclude_reminder_backed=True,
        )
    ...
    for sch in schedules:
        try:
            ...
            logger.error(
                "SCHEDULE_NOTIF: Error processing schedule %s: %s",
...
def _nag_time_for_today(schedule_id: str) -> datetime:
    ...
    but varies day-to-day — same approach as the reminders nag system.
    """
    today = _now().date()
...
def _handle_overdue(sch, schedule_id, title, assigned_to, next_due, notify_channel):
    ...
    logger.info(
        "SCHEDULE_NOTIF: Created overdue notification for %s → %s (%s)",
        schedule_id, assigned_to, when,
    )
