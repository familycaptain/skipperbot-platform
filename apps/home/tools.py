"""
Home App Tools — Item Locator: Track where things are in the household.
...
def list_located_items(location: str = "", category: str = "") -> str:
    ...
    try:
        ...
        for item in items:
            loc = f" \U0001f4cd {item['location']}" if item.get("location") else ""
...
def _format_item(item: dict) -> str:
    ...
    loc = f"\U0001f4cd Location: {item['location']}" if item.get("location") else "\U0001f4cd Location: (not set)"
    sub = f" > {item['sub_location']}" if item.get("sub_location") else ""
...
def delete_item_location(loc_id: str) -> str:
    """Delete a storage location definition.

    Args:
        loc_id: The location ID (iloc-xxx) from list_item_locations.

    Returns:
        Confirmation or error.

    Ack: Deleting location {loc_id}...
    """
    try:
        if not loc_id or not loc_id.strip():
            return "Error: loc_id is required."
        ok = _dl.delete_location(loc_id.strip())
        if ok:
            return f"Location '{loc_id}' deleted."
        return f"Error: Location '{loc_id}' not found."
    except Exception as e:
        return f"Error in delete_item_location: {str(e)}"
