...
function SetupCard({ message }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-6">
      ...
      <p className="text-base font-medium text-default">Connect Home Assistant</p>
      <p className="text-sm text-muted mt-2 max-w-md">
        {message || "Home Assistant isn't configured yet."} Set the URL + token in
        Settings → Automation, then refresh.
      </p>
      <p className="text-xs text-faint mt-3 max-w-md">
...
function EntityRow({ e, onControl, busy }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg surface-card border border-subtle">
      <Icon size={16} className={e.on ? "text-amber-300" : "text-faint"} />
      <div className="min-w-0 flex-1">
        <div className="text-sm text-default truncate">{e.name}</div>
      ...
      {e.toggleable ? (
        ...
        <span className="shrink-0 text-xs text-faint px-2">{e.state}</span>
      )}
    </div>
  );
}
  ...
  if (!status?.connected) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center px-6">
        ...
        <p className="text-sm font-medium text-default">Can't reach Home Assistant</p>
        <p className="text-xs text-faint mt-2 max-w-md break-words">{status?.message}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-end mb-3">
        <button onClick={load} disabled={loading} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs rounded surface-card hover:bg-[var(--ds-raised)] text-default">
          {loading ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />} Refresh
        </button>
      </div>
      {error && <div className="mb-3 text-xs text-rose-400 flex items-center gap-1.5"><AlertCircle size={12} /> {error}</div>}
      {loading && domains.length === 0 && <div className="flex justify-center py-12 text-faint"><Loader2 className="animate-spin" /></div>}
      {!loading && domains.length === 0 && <p className="text-sm text-faint">No entities found.</p>}
      {domains.map((dom) => {
        const meta = DOMAIN_META[dom] || { label: dom };
        return (
          <div key={dom} className="mb-5">
            <h2 className="text-xs font-semibold uppercase tracking-wide text-faint mb-2">{meta.label}</h2>
...
function AliasChip({ text, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full surface-raised text-default text-[11px]">
      {text}
      <button onClick={onRemove} className="text-muted hover:text-rose-300" title="Remove alias">
        <X size={11} />
      </button>
    </span>
  );
...
function DeviceRow({ d, onAddAlias, onRemoveAlias, busy }) {
  ...
  return (
    <div className="px-3 py-2.5 rounded-lg surface-card border border-subtle">
      <div className="flex items-center gap-2">
        <Cpu size={14} className="text-faint shrink-0" />
        <div className="min-w-0 flex-1">
          <div className="text-sm text-default truncate">{d.name || "(unnamed device)"}</div>
          {(d.manufacturer || d.model) && (
            <div className="text-[11px] text-faint truncate">{[d.manufacturer, d.model].filter(Boolean).join(" · ")}</div>
          )}
        </div>
        {busy === d.device_id && <Loader2 size={12} className="animate-spin text-faint" />}
      </div>
      <div className="flex flex-wrap items-center gap-1.5 mt-2">
        ...
        <span className="inline-flex items-center gap-1">
          <input
            ...
            className="w-28 px-2 py-0.5 text-[11px] rounded surface-panel border border-subtle text-default placeholder:text-[var(--ds-faint)]"
          />
          <button onClick={add} className="text-muted hover:text-emerald-300" title="Add alias"><Plus size={13} /></button>
        </span>
      </div>
    </div>
  );
...
function NamesManager() {
  ...
  if (loading) {
    return <div className="flex justify-center py-12 text-faint"><Loader2 className="animate-spin" /></div>;
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={load} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs rounded surface-card hover:bg-[var(--ds-raised)] text-default">
      ...
      {/* Entity aliases */}
      <h2 className="text-xs font-semibold uppercase tracking-wide text-faint mb-2 flex items-center gap-1.5">
        <Tag size={12} /> Entity aliases
      </h2>
      <div className="rounded-lg surface-card border border-subtle p-3 mb-3">
        <div className="flex flex-wrap gap-2 items-end">
          <div className="flex flex-col">
            <label className="text-[10px] text-faint mb-0.5">Name people say</label>
            <input value={form.alias} onChange={(e) => setForm({ ...form, alias: e.target.value })}
              placeholder="tv, kitchen lamp…"
              className="w-40 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default placeholder:text-[var(--ds-faint)]" />
          </div>
          <div className="flex flex-col">
            <label className="text-[10px] text-faint mb-0.5">Home Assistant entity</label>
            <input value={form.entity_id} onChange={(e) => setForm({ ...form, entity_id: e.target.value })}
              ...
              className="w-72 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default placeholder:text-[var(--ds-faint)] font-mono" />
            <datalist id="ha-entities">
              {entities.map((e) => <option key={e.entity_id} value={e.entity_id}>{e.name}</option>)}
            </datalist>
          </div>
          <div className="flex flex-col">
            <label className="text-[10px] text-faint mb-0.5">Notes (optional)</label>
            <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
              className="w-40 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default" />
          </div>
          <button onClick={addAlias} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs rounded bg-emerald-600/80 hover:bg-emerald-600 text-on-accent">
            <Plus size={13} /> Add
          </button>
        </div>
      </div>
      <div className="space-y-1.5 mb-6">
        {aliases.length === 0 && <p className="text-sm text-faint">No aliases yet.</p>}
        {aliases.map((a) => editing === a.alias ? (
          <div key={a.alias} className="flex flex-wrap items-end gap-2 px-3 py-2 rounded-lg surface-card border border-amber-700/40">
            <div className="flex flex-col">
              <label className="text-[10px] text-faint mb-0.5">Name</label>
              <input value={editForm.alias} onChange={(e) => setEditForm({ ...editForm, alias: e.target.value })}
                className="w-36 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default" />
            </div>
            <div className="flex flex-col">
              <label className="text-[10px] text-faint mb-0.5">Entity</label>
              <input value={editForm.entity_id} onChange={(e) => setEditForm({ ...editForm, entity_id: e.target.value })}
                list="ha-entities"
                className="w-64 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default font-mono" />
            </div>
            <div className="flex flex-col">
              <label className="text-[10px] text-faint mb-0.5">Notes</label>
              <input value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })}
                className="w-36 px-2 py-1 text-xs rounded surface-panel border border-subtle text-default" />
            </div>
            <button onClick={() => saveEdit(a.alias)} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs rounded bg-emerald-600/80 hover:bg-emerald-600 text-on-accent" title="Save">
              <Check size={13} /> Save
            </button>
            <button onClick={cancelEdit} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs rounded surface-raised hover:bg-[var(--ds-raised)] text-default" title="Cancel">
              <X size={13} />
            </button>
          </div>
        ) : (
          <div key={a.alias} className="flex items-center gap-3 px-3 py-2 rounded-lg surface-card border border-subtle">
            <div className="min-w-0 flex-1">
              <div className="text-sm text-default">
                <span className="font-medium">{a.alias}</span>
                <span className="text-faint"> → </span>
                <span className="font-mono text-xs text-muted">{a.entity_id}</span>
              </div>
              {a.notes && <div className="text-[11px] text-faint truncate">{a.notes}</div>}
            </div>
            <button onClick={() => startEdit(a)} className="shrink-0 text-faint hover:text-amber-300" title="Edit alias">
              <Pencil size={14} />
            </button>
            <button onClick={() => deleteAlias(a.alias)} className="shrink-0 text-faint hover:text-rose-300" title="Delete alias">
              <Trash2 size={14} />
            </button>
          </div>
        ))}
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        {devices.length === 0 && <p className="text-sm text-faint">No devices cached yet.</p>}
        {devices.map((d) => (
...
export default function AutomationApp() {
  ...
  if (loading && !status) {
    return <div className="flex items-center justify-center h-full text-faint"><Loader2 className="animate-spin" /></div>;
  }
  ...
  return (
    <div className="h-full w-full overflow-y-auto surface-page p-5">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-2 mb-4">
          <Lightbulb className="text-amber-400" size={20} />
          <h1 className="text-lg font-bold text-default">Automation</h1>
          {status?.connected
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium -mb-px border-b-2 transition-colors ${
                  active ? "border-amber-400 text-amber-300" : "border-transparent text-muted hover:text-[var(--ds-text)]"
                }`}>
