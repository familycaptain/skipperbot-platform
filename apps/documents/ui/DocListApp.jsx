    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex items-center justify-between px-3 h-10 surface-panel border-b border-subtle shrink-0">
        <div className="flex items-center gap-1.5 text-sm text-default">
          ...
          <span className="text-default">Documents</span>
        </div>
        <button
          onClick={() => loadList()}
          disabled={loading}
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center surface-card border border-subtle rounded-lg px-3 py-1.5">
          <Search size={14} className="text-faint shrink-0 mr-2" />
          <input
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") onSearch(); }}
            placeholder="Search documents..."
            className="bg-transparent text-sm text-default placeholder-slate-600 outline-none w-full"
          />
          {searchQuery && (
            <button onClick={() => onSearchChange("")} className="text-faint hover:text-[var(--ds-text)] ml-1">
              <X size={12} />
            </button>
          )}
        </div>
        <button onClick={onSearch} disabled={loading} className="p-2 rounded surface-card text-muted hover:text-[var(--ds-text)] hover:bg-[var(--ds-raised)] transition-colors">
          <Search size={14} />
        </button>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (newTitle.trim()) { onCreate(newTitle); setNewTitle(""); } }} className="flex items-center gap-2">
        <div className="flex-1 flex items-center surface-card border border-subtle rounded-lg px-3 py-1.5">
          <Plus size={14} className="text-faint shrink-0 mr-2" />
          <input
            value={newTitle}
        ...
        <button type="submit" disabled={!newTitle.trim()} className="px-3 py-1.5 rounded bg-indigo-600 text-on-accent text-xs font-medium hover:bg-indigo-500 disabled:opacity-30 disabled:cursor-default transition-colors">
          Create
        </button>
      </form>
          ...
          {docs.map((d) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-default truncate">{d.title || d.id}</span>
