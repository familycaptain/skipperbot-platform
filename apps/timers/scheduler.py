...
Cancellation: cancel the task. The task removes itself from the registry in a
finally block.

Graceful shutdown: ``shutdown_all_timers()`` cancels every active task and waits
for them to finish unwinding. No expired timers fire after shutdown begins. It
is invoked from the platform's FastAPI lifespan shutdown (guarded, so the
platform runs fine when this optional app isn't installed).
"""
...
logger = logging.getLogger("apps.timers")
...
async def shutdown_all_timers():
    ...
    Called from the FastAPI lifespan shutdown. After this returns, no pending
    timer can fire — even ones that were milliseconds from expiry.
    """
