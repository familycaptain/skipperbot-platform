...
const SEV_COLOR = { high: "bg-red-900/50 text-red-300", med: "bg-amber-900/50 text-amber-300",
  medium: "bg-amber-900/50 text-amber-300", low: "surface-raised text-default" };
const ATTENTION = new Set(["crit", "lead"]);
// Code Scout "planned change" action badges — rewrite/delete/move read as higher-impact
const ACTION_CLS = {
  add: "bg-emerald-900/50 text-emerald-300", modify: "surface-card text-accent",
...
const STATUS = {
  running:  { cls: "surface-card text-accent", label: "running" },
...
const KIND_CLS = {
  tool: "text-muted", emit: "text-indigo-300", agent_start: "text-accent",
  agent_end: "text-emerald-300", node: "text-default", info: "text-faint",
  error: "text-red-300", text: "text-default",   // the agent's own narration
};

function Field({ k, v }) {
  let body;
  if (typeof v === "string") body = <span className="text-default">{v}</span>;
  else if (Array.isArray(v)) body = (
    <ul className="list-disc ml-4 text-default space-y-0.5">
      {v.map((x, i) => (
        <li key={i}>
          ...
          {x && x.category && <span className="ml-1 text-[10px] text-faint">{x.category}</span>}
        </li>
      ))}
    </ul>
  );
  else if (v && typeof v === "object") body = <span className="text-default">{Object.entries(v).map(([kk, vv]) => `${kk}: ${vv}`).join(" · ")}</span>;
  else body = <span className="text-default">{String(v)}</span>;
  return <div className="text-xs"><span className="text-faint">{k.replace(/_/g, " ")}: </span>{body}</div>;
}
    ...
    <div className="border border-subtle rounded-md">
      <button onClick={() => detailKeys.length && setOpen((x) => !x)} className="w-full text-left px-2.5 py-1.5">
        <div className="flex items-center justify-between">
          ...
          {detailKeys.length > 0 && <span className="text-[11px] text-faint">{open ? "− less" : "+ details"}</span>}
        </div>
        {o.summary && <div className="text-xs text-default mt-0.5 leading-snug">{o.summary}</div>}
      </button>
      {open && (
        <div className="px-2.5 pb-2 border-t border-subtle pt-2 space-y-1.5">
...
function Section({ title, children }) {
  ...
  return (
    <div className="mb-4">
      <div className="text-[11px] uppercase tracking-wide text-faint mb-1">{title}</div>
...
function AgentLog({ name, events, active, done, expandAll }) {
  ...
  return (
    <div className={`border rounded-md ${active ? "border-subtle" : "border-subtle"}`}>
      <div className="flex items-center gap-1.5 px-2.5 py-1 border-b border-subtle">
        {active ? <Circle size={8} className="text-accent fill-sky-400 animate-pulse shrink-0" />
          : done ? <CheckCircle2 size={11} className="text-emerald-500 shrink-0" />
          : <Circle size={8} className="text-faint shrink-0" />}
        <span className="text-xs font-medium">{AGENT_LABEL[name] || name}</span>
        <span className="text-[10px] text-faint ml-auto shrink-0">{events.length} · {active ? "active" : done ? "done" : "idle"}</span>
        {isOpen && (
          <button onClick={() => setTall((t) => !t)} className="text-[10px] text-faint hover:text-[var(--ds-text)] shrink-0" title="taller / shorter">
            {tall ? "▣" : "⤢"}
          </button>
        )}
        <button onClick={() => setOpen((o) => !o)} className="text-faint hover:text-[var(--ds-text)] shrink-0">
          {isOpen ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
        </button>
      </div>
      {isOpen ? (
        <div ref={boxRef} onScroll={onScroll}
             className={`${tall ? "max-h-[32rem]" : "max-h-72"} overflow-y-auto px-2.5 py-1.5 font-mono text-[11px] leading-relaxed bg-[rgb(0_0_0/0.30)]`}>
          {events.length === 0 && <div className="text-faint">waiting…</div>}
          {events.map((e) => (
            <div key={e.id} className={`whitespace-pre-wrap break-words ${KIND_CLS[e.kind] || "text-muted"}`}>{e.message}</div>
          ))}
        </div>
      ) : (
        last && <button onClick={() => setOpen(true)} className="block w-full text-left px-2.5 py-1 font-mono text-[11px] text-faint truncate hover:text-[var(--ds-text)]">{last}</button>
      )}
    </div>
  );
}
...
class EvolveBoundary extends Component {
  ...
  render() {
    if (this.state.err) {
      return (
        <div className="flex flex-col items-center justify-center h-full w-full surface-page text-default p-6 text-center">
          <div className="text-amber-300 font-medium mb-1">Evolve hit a render error</div>
          <div className="text-xs text-muted max-w-md mb-3 font-mono break-words">{String(this.state.err?.message || this.state.err)}</div>
          <button onClick={() => this.setState({ err: null })}
            className="px-3 py-1.5 rounded-md surface-raised hover:bg-[var(--ds-raised)] text-sm">Reload the view</button>
          <div className="text-[11px] text-faint mt-2">A single bad run/packet won't take down the rest — pick a different run if this one keeps erroring.</div>
        </div>
      );
    }
...
function EvolveAppInner({ userId, userRole, refreshKey, onTitle }) {
  ...
  return (
    <div className="flex flex-col h-full w-full surface-page text-default">
      ...
      <div className="flex items-center justify-between px-3 h-8 border-b border-subtle shrink-0">
        <span className="text-[11px] uppercase tracking-wide text-faint flex items-center gap-1.5"><Workflow size={12} /> Evolve</span>
        <span className="text-[11px] flex items-center gap-2">
          <span className="flex items-center gap-1" title="Evolve spend this week (since Monday)">
            <span className="text-faint uppercase tracking-wide">this week</span>
          ...
          <span className="flex items-center gap-1" title="cumulative Evolve spend across all items">
            <span className="text-faint uppercase tracking-wide">all items</span>
      ...
      {/* LEFT: runs */}
      <div className="w-72 shrink-0 border-r border-subtle flex flex-col">
        <div className="flex items-center justify-between px-3 h-10 border-b border-subtle shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={refresh} title="refresh now"
              className="text-faint hover:text-[var(--ds-text)] flex items-center">
              <RefreshCw size={12} />
            </button>
            <button onClick={() => { setShowArchived((v) => !v); setSel(null); }}
              className="text-[10px] text-faint hover:text-[var(--ds-text)] flex items-center gap-1"
              title={showArchived ? "show active runs" : "show archived runs"}>
              {showArchived ? (<><Activity size={11} className="text-accent" /> active</>)
        ...
        <div className="flex-1 overflow-y-auto">
          {runs.length === 0 && (
            <div className="text-faint text-sm text-center mt-8 px-4">{showArchived ? "No archived runs." : "No runs yet. Ingested issues and their builds show up here live."}</div>
          )}
          {runs.map((g) => {
            const active = g.instance_id === sel;
            const st = (g.status === "waiting" && decided[g.instance_id])
              ? decidedChip(decided[g.instance_id])
              : (STATUS[g.status] || { cls: "surface-raised text-default", label: g.status });
            return (
              <div key={g.instance_id} role="button" onClick={() => setSel(g.instance_id)}
                className={`relative group w-full text-left px-3 py-2.5 border-b border-subtle cursor-pointer ${active ? "surface-card" : "hover:bg-[var(--ds-panel)]"}`}>
                <button onClick={(e) => { e.stopPropagation(); archiveRun(g.instance_id, !showArchived); }}
                  className="absolute top-1.5 right-1.5 p-1 rounded text-faint hover:text-[var(--ds-text)] hover:bg-[var(--ds-raised)] opacity-0 group-hover:opacity-100"
                ...
                <div className="flex items-center gap-1.5 mb-0.5 pr-6">
                  ...
                  {g.phase && <span className="text-[10px] text-faint">{g.phase}</span>}
                </div>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto p-4">
        ...
        {!sel && <div className="text-faint mt-10 text-center">Select a run to watch it work.</div>}
        {sel && (
          <div className="max-w-3xl">
            ...
            {!atGate && run?.status === "merged" && isParent && (
              <div className="mb-4">
                ...
                <div className="text-[11px] text-faint mt-1">Re-opens this at the verify gate so you can report what's broken; it resumes the same conversation to fix it.</div>
              </div>
            )}
            ...
            {atGate && (
              <>
                <div className={`mb-2 inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-1 rounded ${gate3 ? "bg-amber-900/40 text-amber-300 border border-amber-700/50" : gate2 ? "bg-emerald-900/40 text-emerald-300 border border-emerald-700/50" : "surface-card text-accent border border-subtle"}`}>
                ...
                <div className={`border rounded-lg p-3 mb-4 ${REC_COLOR[rec.action] || "surface-card border-subtle"}`}>
                ...
                {pkt.related?.length > 0 && (
                  <div className="border border-amber-700/40 bg-amber-900/20 rounded-lg p-2.5 mb-4 text-xs">
                    ...
                    {pkt.related.map((r, i) => (
                      <div key={i} className="text-default">
                        • {r.title || r.instance}{" "}
                        <span className="text-faint">({r.source || r.gate || r.status}; shares {(r.overlap || []).join(", ")})</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {pkt.decisions_needed?.length > 0 && (
                  <Section title="Decisions for you">
                    {pkt.decisions_needed.map((d, i) => (
                      <div key={i} className="border border-subtle surface-card rounded p-2 mb-2">
                        <div className="text-default"><span className="opacity-50 mr-1">{i + 1}.</span>{d.question}</div>
                        {d.recommendation && <div className="text-xs text-accent mt-0.5">→ recommends: {d.recommendation}</div>}
                        {d.options?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1.5">
                            {d.options.map((opt, j) => (
                              <button key={j} disabled={!isParent}
                                onClick={() => setAnswers((a) => ({ ...a, [i]: opt }))}
                                className={`text-[11px] px-2 py-0.5 rounded border disabled:opacity-50 ${answers[i] === opt ? "bg-[var(--ds-accent)] border-subtle text-on-accent" : "border-subtle text-default hover:border-[var(--ds-border)]"}`}>
                                {opt}
                              </button>
                            ))}
                          </div>
                        ...
                        <input type="text" disabled={!isParent} value={answers[i] || ""}
                          ...
                          className="mt-1.5 w-full surface-panel border border-subtle rounded px-2 py-1 text-sm text-default placeholder-slate-600 focus:border-subtle focus:outline-none disabled:opacity-50" />
                      </div>
                    ))}
                  </Section>
                )}
                ...
                {atGate && isParent && (
                  <div className="mb-3">
                    ...
                    <textarea value={guidance} onChange={(e) => setGuidance(e.target.value)} rows={2}
                      ...
                      className="w-full surface-panel border border-subtle rounded px-2 py-1.5 text-sm text-default placeholder-slate-600 focus:border-subtle focus:outline-none" />
                  </div>
                )}
                ...
                {isParent ? (
                  <>
                    <div className="flex gap-2">
                      <button disabled={!!busy} onClick={() => decide("approve")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-700 hover:bg-emerald-600 text-on-accent text-sm disabled:opacity-50">
                      ...
                      <button disabled={!!busy} onClick={() => decide("change")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-amber-700 hover:bg-amber-600 text-on-accent text-sm disabled:opacity-50">
                        {busy === "change" ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />} {gate3 ? "Still broken — send back" : "Change & send answers"}
                      </button>
                      <button disabled={!!busy} onClick={() => decide("reject")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-red-800 hover:bg-red-700 text-on-accent text-sm disabled:opacity-50">
                        {busy === "reject" ? <Loader2 size={14} className="animate-spin" /> : <XCircle size={14} />} {gate3 ? "Abandon" : "Reject"}
                      </button>
                    </div>
                    <div className="text-[11px] text-faint mt-1.5 mb-5">
                      {gate3
                        ...
                        : gate2
                        ? <><span className="text-emerald-300">Approve</span> merges this branch into <span className="text-default">release</span> and publishes it; <span className="text-amber-300">Change</span> sends it back to rebuild; <span className="text-red-300">Reject</span> discards it.</>
                ...
                ) : <div className="text-xs text-faint mb-5">A parent or admin decides this gate.</div>}
              </>
            )}
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-1">
                ...
                {lanes.length > 0 && (
                  <button onClick={() => setExpandAll((x) => !x)} className="text-[10px] text-faint hover:text-[var(--ds-text)]">
                    {expandAll ? "collapse all" : "expand all"}
                  </button>
                )}
              </div>
              {lanes.length === 0 ? (
                <div className="text-faint text-xs">No activity yet for this run.</div>
              ) : (
            ...
            {atGate && (
              <>
                {pkt.work_item?.body && <Section title="Report"><p className="whitespace-pre-wrap text-default">{pkt.work_item.body}</p></Section>}
                ...
                {proposal.spec_id && (
                  <Section title="Proposed spec">
                    ...
                    {proposal.implements?.length > 0 && (
                      <div className="text-xs text-muted mb-2">implements: {proposal.implements.join(", ")}</div>
                    )}
                    {proposal.tests?.length > 0 && proposal.tests.map((t, i) => (
                      <div key={i} className="text-xs text-default flex gap-1.5 mb-1">
                        <FlaskConical size={12} className="mt-0.5 shrink-0 text-faint" />
                        <span><span className="font-mono text-muted">{t.path || t.type}</span>{t.rubric ? <span className="whitespace-pre-wrap"> — {t.rubric}</span> : null}</span>
                      </div>
                    ))}
                    {proposal.notes && <div className="text-xs text-faint mt-2 whitespace-pre-wrap">{proposal.notes}</div>}
                  </Section>
                )}

                {codePlan && !gate2 && !gate3 && (
                  <Section title="Planned code changes (read-only sketch — no code written yet)">
                    {codePlan.summary && <div className="text-sm text-default mb-1">{codePlan.summary}</div>}
                    {codePlan.approach && <div className="text-xs text-muted mb-2 whitespace-pre-wrap">{codePlan.approach}</div>}
                      ...
                      <div className="space-y-1 mb-2">
                        {codePlan.changes.map((c, i) => (
                          <div key={i} className="text-xs flex gap-1.5 items-baseline">
                            <span className={`text-[9px] uppercase px-1 rounded shrink-0 ${ACTION_CLS[c.action] || "surface-raised text-default"}`}>{c.action}</span>
                            <span className="font-mono text-indigo-300 shrink-0">{c.path}</span>
                            {c.what && <span className="text-muted">— {c.what}</span>}
                          </div>
                        ))}
                      </div>
                    )}
                    {Array.isArray(codePlan.new_modules) && codePlan.new_modules.length > 0 && (
                      <div className="text-xs text-muted mb-1.5">
                        <span className="text-faint">new modules: </span>
                    ...
                    {Array.isArray(codePlan.placement_notes) && codePlan.placement_notes.length > 0 && (
                      <div className="border border-amber-700/40 bg-amber-900/15 rounded p-2 mb-1.5">
                        ...
                        <ul className="list-disc ml-4 text-xs text-default space-y-0.5">
                          {codePlan.placement_notes.map((n, i) => <li key={i}>{n}</li>)}
                        </ul>
                      </div>
                    )}
                    {Array.isArray(codePlan.risks) && codePlan.risks.length > 0 && (
                      <div className="text-xs mb-1"><span className="text-faint">risks: </span>
                        <ul className="list-disc ml-4 text-default space-y-0.5">{codePlan.risks.map((r, i) => <li key={i}>{r}</li>)}</ul>
                      </div>
                    )}
                    {Array.isArray(codePlan.open_questions) && codePlan.open_questions.length > 0 && (
                      <div className="text-xs"><span className="text-faint">open questions: </span>
                        <ul className="list-disc ml-4 text-muted space-y-0.5">{codePlan.open_questions.map((q, i) => <li key={i}>{q}</li>)}</ul>
                      </div>
                    )}
                  </Section>
                )}
                  ...
                  <Section title={`Spec tree (${pkt.spec_tree.length} specs)`}>
                    {pkt.spec_tree.map((s, i) => (
                      <div key={i} className="text-xs mb-1.5">
                        ...
                        {s.summary && <div className="text-faint ml-2 mt-0.5">{s.summary}</div>}
                      </div>
                    ))}
                  </Section>
                )}
                ...
                {pkt.diff && (
                  <Section title="Diff">
                    <pre className="text-[11px] bg-[rgb(0_0_0/0.40)] rounded p-2 overflow-x-auto max-h-96 text-default">{pkt.diff}</pre>
                  </Section>
                )}

                {pkt.feature?.branch && (
                  <div className="text-xs text-faint flex items-center gap-1.5 mt-2"><GitBranch size={12} /> {pkt.feature.branch}</div>
                )}
              </>
            )}
          </div>
