...
const STATUS_STYLES = {
  ...
  running: { icon: Loader2, color: "text-sky-400", bg: "bg-sky-900/20", label: "Running" },
  skipped: { icon: MinusCircle, color: "text-slate-500", bg: "bg-slate-800/30", label: "Skipped" },
};
...
function BackupSection({ sourceKey, meta, state, onRefresh, onRunNow, onToggle, onDelete }) {
  ...
  return (
    <section className="flex flex-col min-h-[380px] rounded-xl border border-slate-800 bg-slate-950/40 overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-800 bg-slate-900/35">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-xs bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 text-white rounded transition-colors"
          >
      ...
      {config && available && (
        <div className="flex items-center gap-3 px-3 py-2 bg-slate-900/20 border-b border-slate-800/50 text-xs shrink-0">
          <button
            onClick={onToggle}
          ...
          >
            <div className={`w-2 h-2 rounded-full ${config.enabled ? "bg-emerald-400" : "bg-slate-600"}`} />
      ...
      {latest && available && (
        <div className="px-3 py-2 border-b border-slate-800/30 bg-slate-900/10 shrink-0">
...
function UnavailableState({ error }) {
  return (
    <div className="flex flex-col items-center justify-center h-40 text-center text-slate-500 px-4">
      ...
      <div className="text-sm text-slate-300">Backup source unavailable</div>
      <div className="mt-1 text-[11px] text-slate-500 max-w-sm">
...
function BackupCard({ backup: b, onDelete }) {
  ...
  return (
    <div className={`rounded-lg border border-slate-700/50 ${s.bg} transition-colors`}>
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm text-white font-medium">{fmtDate(b.started_at)}</span>
            <span className="text-[10px] text-slate-500">{fmtTime(b.started_at)}</span>
            <span className={`text-[10px] px-1.5 py-0.5 rounded ${s.bg} ${s.color} border border-current/20`}>
              {s.label}
            </span>
            {b.duration_secs > 0 && (
              <span className="text-[10px] text-slate-500">{fmtDuration(b.duration_secs)}</span>
            )}
          </div>

          {b.status === "completed" && (
            <div className="mt-0.5 text-[11px] text-slate-500 flex items-center gap-3">
          ...
          {b.status === "skipped" && (
            <div className="mt-0.5 text-[11px] text-slate-600 italic">Backups were disabled</div>
          )}
        </div>
      </button>
      ...
      {expanded && b.status === "completed" && (
        <div className="border-t border-slate-700/30 px-3 pb-3 pt-2 space-y-2">
          {topCounts.length > 0 && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-400">
              {topCounts.map(([table, count]) => (
                <span key={table}>
                  <span className="text-slate-500">{table}:</span> {count.toLocaleString()}
                </span>
              ))}
            </div>
          )}
          ...
          {b.network_path && (
            <div className="text-[11px] text-slate-500 truncate" title={b.network_path}>
              {b.network_path}
            </div>
          )}
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-slate-700/30">
            <span className="text-[10px] text-slate-600 font-mono">{b.id}</span>
      ...
      {expanded && b.status === "failed" && b.error && (
        <div className="border-t border-slate-700/30 px-3 pb-3 pt-2">
          ...
          <div className="flex items-center gap-2 pt-2 mt-2 border-t border-slate-700/30">
            <span className="text-[10px] text-slate-600 font-mono">{b.id}</span>
            <div className="flex-1" />
            <button
              onClick={(e) => { e.stopPropagation(); onDelete(b.id); }}
