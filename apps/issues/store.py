"""Issues App — Business Logic

Create, update, and list issues. Sends notifications on key events.
"""
...
from apps.issues import data as _dl
...
def create_issue(
    ...
    if not _users.has_role(reporter or {}, "admin"):
        icon = "\U0001fab2" if issue_type == "bug" else "\u2728"  # 🪲 or ✨
        admins = _users.get_users_with_any_role("admin")
        for admin in admins:
            create_notification(
...
def update_issue(
    ...
    if new_status != old_status:
        ...
        if old_status == "pending_validation" and new_status == "fixed" and updated_by != "user":
            create_notification(
                recipient="user",
