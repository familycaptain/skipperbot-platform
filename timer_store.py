"""
SkipperBot Timer Store
======================
In-memory registry of active timers. Timers are short-lived (seconds to
minutes), so they live only in process memory — they evaporate on restart.
The notification that fires when a timer expires is the persisted artifact.

State is mutated only from the asyncio event loop (timer tasks and async
tool functions), so no locking is required.
"""
...
def register(timer_id: str, user_id: str, name: str, duration_seconds: int, task) -> dict:
    ...
    record = {
        ...
        "expires_at": now.replace(microsecond=0).isoformat(),  # placeholder, set below
        "task": task,
    }
    # Compute expires_at precisely
    from datetime import timedelta
    record["expires_at"] = (now + timedelta(seconds=duration_seconds)).isoformat()
