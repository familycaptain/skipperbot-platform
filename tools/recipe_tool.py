"""DEPRECATED — Moved to apps/recipes/tools.py (app package).
This file is no longer imported. Safe to delete.

Original: Recipe Tools — Create and manage family recipes.
...
from data_layer import recipes as _dl_recipes
...
def create_recipe(
    ...
    try:
        ...
        _dl_recipes.save_recipe(recipe)
...
def get_recipe(recipe_id: str) -> str:
    ...
    try:
        ...
        recipe = _dl_recipes.get_recipe(recipe_id.strip())
        if not recipe:
...
def list_recipes(category: str = "", created_by: str = "") -> str:
    ...
    try:
        if category and category.strip():
            recipes = _dl_recipes.filter_by_category(category.strip())
        else:
            recipes = _dl_recipes.get_all_recipes()
...
def search_recipes(query: str) -> str:
    ...
    try:
        ...
        recipes = _dl_recipes.search_recipes(query.strip())
        if not recipes:
...
def update_recipe(
    ...
    try:
        ...
        success = _dl_recipes.update_recipe(recipe_id.strip(), updates)
        if success:
...
def delete_recipe(recipe_id: str) -> str:
    ...
    try:
        ...
        success = _dl_recipes.delete_recipe(recipe_id.strip())
        if success:
...
def list_recipe_categories() -> str:
    ...
    """
    try:
        cats = _dl_recipes.get_all_categories()
        if not cats:
...
def create_recipe_category(name: str) -> str:
    ...
    try:
        ...
        cat = _dl_recipes.create_category(cat_id, name.strip())
        if cat:
...
def update_recipe_category(category_id: str, name: str) -> str:
    ...
    try:
        ...
        success = _dl_recipes.update_category(category_id.strip(), name.strip())
        if success:
...
def delete_recipe_category(category_id: str) -> str:
    ...
    try:
        ...
        success = _dl_recipes.delete_category(category_id.strip())
        if success:
